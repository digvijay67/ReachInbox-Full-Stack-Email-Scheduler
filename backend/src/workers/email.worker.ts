...
async function recoverStaleProcessingEmails() {
  try {
    ...
    console.log(
      `Found ${staleEmails.length} stale PROCESSING email(s), checking recovery...`
    );
    ...
    for (const email of staleEmails) {
      ...
      const reset = await prisma.email.updateMany({
        where: {
          id: email.id,
          status: "PROCESSING",
        },
    ...
    console.error(
      "Startup recovery of stale PROCESSING emails failed:",
      error
    );
  }
}
 ...
 */
async function recoverOrphanedScheduledEmails() {
  try {
    const now = new Date();
    ...
    console.log(
      `Found ${dueEmails.length} overdue SCHEDULED email(s), checking for live jobs...`
    );

    for (const email of dueEmails) {
      const jobId =
        email.jobId || `email-${email.id}`;
